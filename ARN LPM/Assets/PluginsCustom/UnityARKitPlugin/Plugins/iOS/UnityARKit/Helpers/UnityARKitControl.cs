...
namespace UnityEngine.XR.iOS
{
    public class UnityARKitControl : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            {
                ARKitWorldTrackingSessionConfiguration sessionConfig =
                    new ARKitWorldTrackingSessionConfiguration(alignmentOptions[currentAlignmentIndex],
                        planeOptions[currentPlaneIndex]
                    );

                UnityARSessionNativeInterface.GetARSessionNativeInterface().RunWithConfigAndOptions(
                    sessionConfig, runOptions[currentOptionIndex]
                );
            }
            ...
            string runOptionStr = (
                ...
                "Full" : (currentOptionIndex == 1 ?
                    "Tracking" : (currentOptionIndex == 2 ?
                        "Anchors" : "None"))
            );
